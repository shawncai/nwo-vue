<template>
  <div class="install-container">
    <div class="filter-container">
      ...
      <el-button v-permission="[4]" v-waves class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">{{ $t('repair.add') }}</el-button>
    </div>
    <div class="app-container">
      <el-tabs v-model="activeName">
        <el-tab-pane :label="$t('repair.all')" name="first">
          <el-table
            ...
            style="width: 100%;">
            <el-table-column :label="$t('repair.coding')" prop="code" align="center" min-width="150">
              <template slot-scope="scope">
                <span>{{ scope.row.code }}</span>
              </template>
            </el-table-column>cnpm install node-sass@latest
            <el-table-column :label="$t('repair.customername')" prop="personName" width="140px" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.personName }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('repair.phone')" align="center" prop="phoneNumber" width="140px">
              <template slot-scope="scope">
                <span>{{ scope.row.phoneNumber }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('repair.Repairaddress')" prop="address" min-width="220px" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.address }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('repair.Warrantytime')" prop="createTime" width="160px" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.createTime }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('repair.stat')" prop="stat" min-width="110px" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.stat | zhuang }}</span>
              </template>
            </el-table-column>
            <el-table-column v-permission="[5, 6]" :label="$t('table.actions')" align="center" min-width="180" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button v-permission="[5]" type="primary" size="mini" @click="handleDetail(scope.row)">{{ $t('table2.view') }}</el-button>
                <el-button v-permission="[93]" v-if="scope.row.stat == 1" type="danger" size="mini" @click="handleDelete(scope.row)">{{ $t('Complaint.delete') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 1" size="mini" type="warning" @click="handleedit(scope.row)">{{ $t('repair.edit2') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 2" size="mini" type="info" @click="handlecancel(scope.row)">{{ $t('repair.cancel') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 5" size="mini" type="info" @click="handlefinish(scope.row)">{{ $t('repair.terminate') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 1" size="mini" type="success" style="width: 70px;" @click="handleDispatch(scope.row)">{{ $t('repair.Dispatch') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat !== 1" size="mini" style="width: 80px;" plain >{{ $t('repair.Dispatched') }}</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <el-table
            ...
            <el-table-column v-permission="[5, 6]" :label="$t('table.actions')" align="center" min-width="180" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button v-permission="[5]" type="primary" size="mini" @click="handleDetail(scope.row)">{{ $t('table2.view') }}</el-button>
                <el-button v-permission="[93]" v-if="scope.row.stat == 1" type="danger" size="mini" @click="handleDelete(scope.row)">{{ $t('Complaint.delete') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 1" size="mini" type="success" @click="handleedit(scope.row)">{{ $t('repair.edit2') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 1" size="mini" type="success" style="width: 70px;" @click="handleDispatch(scope.row)">{{ $t('repair.Dispatch') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat !== 1" size="mini" style="width: 80px;" plain >{{ $t('repair.Dispatched') }}</el-button>
              </template>
            </el-table-column>
          ...
          <el-table
            ...
            <el-table-column v-permission="[5, 6]" :label="$t('table.actions')" align="center" min-width="180" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button v-permission="[5]" type="primary" size="mini" @click="handleDetail(scope.row)">{{ $t('table2.view') }}</el-button>
                <el-button v-permission="[93]" v-if="scope.row.stat == 1" type="danger" size="mini" @click="handleDelete(scope.row)">{{ $t('Complaint.delete') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 1" size="mini" type="success" @click="handleedit(scope.row)">{{ $t('repair.dispatch') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 5" size="mini" type="info" @click="handlefinish(scope.row)">{{ $t('repair.terminate') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat == 1" size="mini" type="success" style="width: 70px;" @click="handleDispatch(scope.row)">{{ $t('repair.Dispatch') }}</el-button>
                <el-button v-permission="[6]" v-if="scope.row.stat !== 1" size="mini" style="width: 80px;" plain >{{ $t('repair.Dispatched') }}</el-button>
              </template>
      ...
      >
        ...
        <el-row style="margin-top: 15px">
          <el-card class="box-card">
            <div slot="header" class="clearfix">
              ...
              <el-button style="float: right; padding: 3px 0;color: #f56c6c" type="text" @click="trajectory(details.employeeId, details.serviceStartTime)">{{ $t('employee.employeetracks') }}</el-button>
            </div>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="centerDialogVisible = false">{{ $t('repair.cancel') }}</el-button>
          <el-button type="primary" @click="centerDialogVisible = false">{{ $t('repair.ok') }}</el-button>
        </span>
      </el-dialog>
      <el-dialog :visible.sync="isvisible" :title="$t('repair.Dispatch2')" width="30%" center lock-scroll>
      ...
      <el-dialog :visible.sync="dialogFormVisible" :title="$t('repair.Add')" width="50%" center lock-scroll top="-10px">
        <el-form :model="form" style="width: 400px; margin-left:50px;">
          ...
          <el-form-item :label-width="formLabelWidth" :label="$t('repair.ExpectedTime')">
            <el-input v-model="form.servicexpecttime"/>
          </el-form-item>
      ...
      <el-dialog :visible.sync="editisvisible" :title="$t('repair.edit')" width="30%" center lock-scroll>
...
<script>
import { stafflist } from '@/api/order'
import { repairList, repairDetail, repairtoemp, producttype, repositorylist, newservice, getserviceschedule, deleteservice, retoreposervice, serviceretoemp, endservicebackground } from '@/api/repair'
...
export default {
  ...
  methods: {
    ...
    handleDispatch(row) {
      ...
      this.isvisible = true
      const regionid = this.$store.getters.regionid
      stafflist(row.expectRepositoryId, regionid).then(res => {
    ...
    trajectory(val, date) {
      ...
      if (date === null) {
        ...
        this.fuzhi = val
        this.fuzhidate = date.substr(0, 10)
