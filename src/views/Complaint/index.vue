<template>
  <div class="tab-container">
      ...
      <el-tab-pane :label="$t('Complaint.all')" name="first">
        <div class="app-container">
          <el-table
            ...
            style="width: 100%;">
            <el-table-column :label="$t('Complaint.id')" prop="id" align="center" width="120">
              <template slot-scope="scope">
                <span>{{ scope.row.id }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('Complaint.userName')" prop="userName" min-width="120px" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.userName }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('Complaint.phone')" align="center" prop="phone" min-width="80px">
              <template slot-scope="scope">
                <span>{{ scope.row.phone }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('Complaint.subject')" prop="subject" min-width="160px" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.subject }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('Complaint.createTime')" prop="createTime" min-width="110px" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.createTime }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('Complaint.handlerName')" align="center" prop="handlerName" min-width="80px">
              <template slot-scope="scope">
                <span>{{ scope.row.handlerName }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('Complaint.stat')" align="center" prop="stat" min-width="80px">
              <template slot-scope="scope">
                <span>{{ scope.row.stat | zhuan }}</span>
              </template>
            </el-table-column>
            <el-table-column v-permission="[16, 17, 18]" :label="$t('Complaint.actions')" align="center" min-width="230" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button v-permission="[18]" v-if="scope.row.stat === 2" size="mini" type="success" style="width: 80px" @click="handleDispatch(scope.row)">{{ $t('Complaint.dispatch') }}</el-button>
                <el-button v-permission="[18]" v-if="scope.row.stat !== 2 && scope.row.stat !== 7" size="mini" style="width: 80px" plain >{{ $t('Complaint.Dispatched') }}</el-button>
                <el-button v-if="scope.row.stat === 7" size="mini" style="width: 80px" plain >{{ $t('Complaint.finish') }}</el-button>
                <el-button v-permission="[17]" type="primary" size="mini" style="width: 80px" @click="handleUpdate(scope.row)">{{ $t('Complaint.seen') }}</el-button>
                <el-button v-permission="[16]" type="danger" size="mini" style="width: 80px" @click="handleDelete(scope.row)">{{ $t('Complaint.delete') }}</el-button>
              </template>
            </el-table-column>
          </el-table>
      ...
      <el-tab-pane :label="$t('Complaint.toassign')" name="second">
        <div class="app-container">
          <el-table
            ...
            highlight-current-row
            style="width: 100%;">
            <el-table-column :label="$t('Complaint.id')" prop="id" align="center" width="60">
              <template slot-scope="scope">
                <span>{{ scope.row.id }}</span>
              </template>
            </el-table-column>
    ...
    <el-dialog :visible.sync="addformVisible" :title="$t('Complaint.addComplaint')" width="30%" center lock-scroll>
    ...
    <el-dialog :visible.sync="isvisible" :title="$t('Complaint.dispatching')" width="30%" center lock-scroll>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="centerDialogVisible = false">{{ $t('Complaint.cancel') }}</el-button>
        <el-button type="primary" @click="centerDialogVisible = false">{{ $t('Complaint.ok') }}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { listcomplaints, getremplist, complainttoemp, usersearch, createcomplaint, complaintdetailemp, deletecomplaint } from '@/api/complaint'
  ...
  methods: {
    ...
    handleFilter() {
      ...
      if (this.activeName === 'first') {
        ...
        this.listQuery.username = this.searchQuery.username
        this.listQuery.phone = this.searchQuery.phone
        listcomplaints(this.listQuery.username, this.listQuery.phone, this.listQuery.stat, this.listQuery.pagenum, this.listQuery.pagesize).then(response => {
          this.list = response.data.data.content.list
          this.total = response.data.data.content.totalCount
          setTimeout(() => {
        ...
        this.listQuery2.username = this.searchQuery.username
        this.listQuery2.phone = this.searchQuery.phone
        ...
        this.listQuery3.username = this.searchQuery.username
        this.listQuery3.phone = this.searchQuery.phone
        ...
        this.listQuery4.username = this.searchQuery.username
        this.listQuery4.phone = this.searchQuery.phone
