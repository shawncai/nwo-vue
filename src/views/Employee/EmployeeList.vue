<template>
  <div class="tab-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.regionid" :placeholder="$t('employee.area')" clearable class="filter-item" style="width: 130px" filterable>
        <el-option v-for="item in arealist" :key="item.id" :label="item.regionName" :value="item.id"/>
      </el-select>
    ...
    <div class="app-container">
      <el-table
        ...
        style="width: 100%;">
        <el-table-column :label="$t('employee.Jobnumber')" prop="id" align="center" width="120">
          <template slot-scope="scope">
            <span>{{ scope.row.id }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('employee.personName')" prop="personName" min-width="190px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.personName }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('employee.repositoryName')" align="center" prop="repositoryName" min-width="200px">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('employee.birthday')" prop="birthday" min-width="110px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.birthday }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('employee.gender')" prop="gender" min-width="60px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.gender | sex }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('employee.createTime')" prop="createTime" min-width="120px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.createTime }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('employee.roleName')" prop="roleName" min-width="150px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.roleName }}</span>
          </template>
        </el-table-column> <el-table-column :label="$t('employee.stat')" prop="stat" min-width="100px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.stat | stat }}</span>
          </template>
        </el-table-column>
        <el-table-column v-permission="[46, 47]" :label="$t('table.actions')" align="center" min-width="150" class-name="small-padding fixed-width">
          <template slot-scope="scope">
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="$t('employee.editemployee')" width="50%" center lock-scroll top="-10px">
    ...
    <el-dialog :visible.sync="dialogFormVisible2" :title="$t('employee.addemployees')" width="50%" center lock-scroll top="-10px">
    ...
    <el-dialog
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible3 = false">{{ $t('employee.cancel') }}</el-button>
        <el-button type="primary" @click="dialogFormVisible3 = false">{{ $t('employee.ok') }}</el-button>
      </div>
    </el-dialog>
    <el-dialog
...
<script>
import { allstore, allarea, editemployee, editarea, getrepositorylist, getrolelist, updateemprepository, addemp, searchemployeelist, gettasklistbyday } from '@/api/employee'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      searchemployeelist(this.listQuery).then(response => {
    ...
    getAllStores() {
      ...
      allarea(repositoryid, regionid).then(res => {
    ...
    handleFilter() {
      ...
      searchemployeelist(this.listQuery).then(res => {
